...
//php inside JsExpression 
$alert_date = yii::t('app','Tgl. Transaksi tidak boleh kosong.');
$ajaxOptions    = [
                    'type' => 'POST',
                    'url'  => $url2,
                    'data' => array(
                    ...
                    'beforeSend' => new yii\web\JsExpression('function(){ 
							if($(\'#dynamicmodel-tgltransaksi\').val() == \'\'){
								...
								$(\'#warning-scanbarcode\').html(alert_date);
...
?>
<input type="hidden" value="ep" name="for">
	...
	<h3>
		...
		<div class="pull-right col-md-6 row" style="">
			...
			echo $form->field($model, 'tglTransaksi', [
						'template' => "{label}<div class='col-md-7'>{input}</div>\n{hint}\n{error}",
						//'labelOptions' => [ 'class' => 'col-md-6' ]
    						...
    						])->label('<small>'.yii::t('app','Tgl Transaksi : ').'</small>',['style'=>'padding-top:0; font-size:17pt;','class'=>'col-md-5']);
