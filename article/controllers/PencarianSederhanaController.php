    ...
    public function actionIndex() {
        ...
        if (Yii::$app->request->get() && $_GET['action'] === "pencarianSederhana") {              
           ...
           /* if (!isset($_GET['page']) || (!isset($_SESSION['countSearch']))) {
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaOpac(:keyword,:ruas,:bahan1,:fAuthor,:fPublisher,:fPublishLoc,:fPublishYear,:fSubject,:fBahasa,:dariTGL,:sampaiTGL,'' );");
                 $command->bindValues($params);
                $command->execute();                
            } else {
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaOpac0(:keyword,:ruas,:bahan1,:limitAwal,:limit,:fAuthor,:fPublisher,:fPublishLoc,:fPublishYear,:fSubject,:fBahasa,:dariTGL,:sampaiTGL,'' );");
                 $command->bindValues($params2);
                $command->execute();
            }*/

            if (!isset($_GET['page']) || (!isset($_SESSION['countSearch']))) {
                $params = [':keyword' => $Keyword, ':ruas' => $ruas, ':isLimit' => 0,':limitAwal' => $limitAwal, ':limitAkhir' => $limit];
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaArticle (:keyword,:ruas,:isLimit,:limitAwal,:limitAkhir);");
                $command->bindValues($params);
                $command->execute();
            } else {
                $params = [':keyword' => $Keyword, ':ruas' => $ruas, ':isLimit' => 1,':limitAwal' => $limitAwal, ':limitAkhir' => $limit];
                $command = Yii::$app->db->createCommand("CALL insertTempSederhanaArticle (:keyword,:ruas,:isLimit,:limitAwal,:limitAkhir);");
                $command->bindValues($params);
                $command->execute();
            ...
            $req=array(
                ...
                'fSubject' => $fSubject,
                'fBahasa' => $fBahasa);
            //jika ga ada tempporary folder (1st search)
            // if ($CID){
                // $count = Yii::$app->db->createCommand("select count(1) from tempCariArticle where CatalogId=".$CID." ")->queryScalar();
                // $hasilSearch = Yii::$app->db->createCommand("select * from tempCssariArticle where CatalogId=".$CID." limit 0,$limit")->queryAll();
            ...
            // } else {
                //OpacHelpers::countSearch($req);
                //$count = Yii::$app->db->createCommand("select count(1) from tempCariArticle")->queryScalar();
                //$hasilSearch = Yii::$app->db->createCommand("select * from tempCariArticle limit 0,$limit")->queryAll();
            //     $count = OpacHelpers::articleSearch($req,'count',$limit);
            //     $hasilSearch = OpacHelpers::articleSearch($req,'hasil',$limit);
            // }

// echo '<pre>';print_r($count);
// echo '<pre>';print_r($hasilSearch);echo die;
            ...
            $dataFacedAuthor = OpacHelpers::facedOpac('Author',$req,'article');
            $dataFacedAuthorLabel = OpacHelpers::facedGenerator(OpacHelpers::facedOpac('Author',$req,'article'),'Author');
            ...
            // echo '<pre>'; print_r($dataFacedAuthor);echo '</pre>';
            ...
            foreach ($hasilSearch as $key => $value) {
                ...
                $dataSearch[$key]['title_article'] =  OpacHelpers::highlight($dataSearch[$key]['title_article'],$dataSearch[$key]['keyword']);
            ...
            return $this->render('resultListOpac', [
                ...
                'dataFacedAuthorLabel' => $dataFacedAuthorLabel,
        ...
        if ($request->isAjax && $_GET['action'] === "showKontenDigital") {
            ...
            //OpacHelpers::print__r($dataCollectionList);
