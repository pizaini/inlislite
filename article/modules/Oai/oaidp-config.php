...
// By default KELEMBAGAAN_SERVER_NAME is taken from the $_SERVER array. If it is different than your
// expected name, please change its value to your actual server name such as: my.server.name.com
define('OPAC', $_SERVER['SERVER_NAME']);

define('OPAC_URL', 'http://'.OPAC.dirname($_SERVER['SCRIPT_NAME']));
...
// MUST (multiple)
// please adjust
$adminEmail			= array('rico.ulul@gmail.com'); 
 ...
 */
//define('MAXRECORDS',$sysconf['OAI']['ListRecords']['RecordPerSet']);
 ...
 * a ResumptionToken will be generated.
 */
//define('MAXIDS',$sysconf['OAI']['ListRecords']['RecordPerSet']);
define('MAXIDS',100);
...
// define all supported sets in your repository
$SETS = 	array (
				array('setSpec'=>'DEMO', 'setName'=>'DEMO')
			);
...
$get_db = explode('dbname', \Yii::$app->db->dsn);
// echo'<pre>';print_r($get_db);die;
$db_host = explode('=',$get_db[0]);
$db_name = explode(';', $get_db[1]);

$DB_HOST   = str_replace(';','',$db_host[1]);
$DB_USER   = \Yii::$app->db->username;
$DB_PASSWD = \Yii::$app->db->password;
$DB_NAME   = str_replace('=','',$db_name[0]);
...
$oaiprefix = "oai".$delimiter.$repositoryIdentifier; 
//$oaiprefix = "";
