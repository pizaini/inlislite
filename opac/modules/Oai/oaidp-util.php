...
function checkArgs($args, $checkList) {
    ...
    $METADATAFORMATS = $GLOBALS['METADATAFORMATS'];


//	global $errors, $TOKEN_VALID, $METADATAFORMATS;
	//global $errors,  $METADATAFORMATS; ASLI
//	$verb = $args['verb'];
	unset($args["verb"]);
...
function selectallQuery ($metadPrefix = "oai_dc", $id = '')
{
	...
	$query = "SELECT * FROM ".$SQL['table']. " WHERE 1 ";
	if ($id != '') {
...
function rowCount($metadataPrefix, $extQuery, $db) {
	global $SQL;
	$n = 0;
	$sql = "SELECT COUNT(*) FROM catalogs WHERE isOpac = 1 " . $extQuery;
	//$sql = "SELECT COUNT(*) FROM ".$SQL['table'] . $extQuery;
	if ($res = $db->query($sql)) {
  		$n = $res->fetchColumn();
