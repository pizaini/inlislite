...
if (empty($errors)) {
	...
	$query2 = "select distinct c.ID from catalogs c where c.isOpac = 1 order by ID ASC";
	...
	if (isset($deliveredrecords)){
		...
		$query2 = "select distinct c.ID from catalogs c where c.isOpac = 1 order by ID ASC LIMIT " . MAXRECORDS . " OFFSET $deliveredrecords ";
		$res = $db->prepare($query2, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
		$r = $res->execute();
...
while ($countrec++ < $maxrec) {
	...
	$datestamp = $record[$SQL['datestamp']];
