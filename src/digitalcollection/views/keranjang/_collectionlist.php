...
<tbody>
		...
		if($dataCollectionList[$i]['BookingMemberID'] == $noAnggota && $dataCollectionList[$i]['BookingExpiredDate'] > $dateNow->format("Y-m-d H:i:sO") ){
			$dataCollectionList[$i]['ketersediaan'] = "Sudah Anda Booking";
			$dataCollectionList[$i]['akses'] = "Di Booking";
			} else
			if($dataCollectionList[$i]['BookingExpiredDate'] > $dateNow->format("Y-m-d H:i:s") ){
			$dataCollectionList[$i]['ketersediaan'] = "Sudah Di Booking";
			$dataCollectionList[$i]['akses'] = "Di Booking";
			}
		if($dataCollectionList[$i]['ketersediaan'] == "Tersedia" && ($dataCollectionList[$i]['akses'] == "Dapat dipinjam" || $dataCollectionList[$i]['akses'] == "Tersedia") && Yii::$app->config->get('IsBookingActivated')=='TRUE')
		{			
			$booking="<form>
		...
		<tr>
			...
			<td>
				...
				echo $dataCollectionList[$i]['ketersediaan'];
				echo $booking;
