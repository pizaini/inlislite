 ...
 *
 *
 * Modified :  change global variabel to globals <http://php.net/manual/en/reserved.variables.globals.php>, 
 * Updateby :  Henry <alvin_vna@yahoo.com>
 * 
 */
...
if (empty($errors)) {
	...
	//identifier from catalogs ID
	$query2 = "select distinct serial_articles.id from serial_articles where serial_articles.isOpac = 1 order by id ASC";
	$res = $db->prepare($query2, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
	$r = $res->execute();
	...
	if (isset($deliveredrecords)){
		...
		//identifier after got resumtipn token
		$query2 = "select distinct serial_articles.id from serial_articles where isOpac = 1 order by id ASC LIMIT " . MAXRECORDS . " OFFSET $deliveredrecords ";
		$res = $db->prepare($query2, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
		$r = $res->execute();
...
while ($countrec++ < $maxrec) {
	...
	$datestamp = $record[$SQL['CreateDate']];
	$setspec = $record[$SQL['set']];
	$subject = $record[$SQL['subject']];
