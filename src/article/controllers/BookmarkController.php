...
class BookmarkController extends \yii\web\Controller {
    ...
    public function actionIndex() {
        ...
        if (/* $request->isAjax  && */isset($_POST['action']) && $_POST['action'] === "Hapus") {
            ...
            if (!isset($_POST['catID'])) {
                Yii::$app->getSession()->setFlash('error', [
                    ...
                    'duration' => 2500,
                    'icon' => 'glyphicon glyphicon-ok-sign',
                    'message' => Yii::t('app', '  Anda harus memilih judul buku yang akan di hapus'),
                    'title' => 'Gagal',
                    'positonY' => Yii::$app->params['flashMessagePositionY'],
                    'positonX' => Yii::$app->params['flashMessagePositionX']
                ]);
        ...
        if (isset($_POST['action']) && $_POST['action'] === "Kosongkan Tampung") {
            $_SESSION['catIDmerge'] = null;
            $_SESSION['catID'] = null;
            Yii::$app->getSession()->setFlash('success', [
                'type' => 'info',
        ...
        foreach ($modelBookmark as $value) {
            ...
            $dataTagRDA        = OpacHelpers::getTaginfo($dataBookmark[$temp]['id'],'336,338','a');
            $jenisBahanRDA     = OpacHelpers::jenisBahanRDA($dataTagRDA);
            $jenis_bahanold    = $dataBookmark[$temp]['worksheet'];
            $dataBookmark[$temp]['worksheet'] = $jenis_bahanold." ".$jenisBahanRDA;
            $temp++;
        }
